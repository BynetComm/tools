...
if "%4" == "" (if "%pathtesthost%"=="" set pathtesthost= 172.17.50.252) else (set pathtesthost=%4) 
...
@c:\bynet\tools\net\pathtest.exe %pathtesthost% -b %pathtestbw% --packets %pathtestpackets% %pathtestdirecton% %8 %9 > %pathtestfilepathname%%pathtestfileindex%.txt
set /a index=%index% +1
